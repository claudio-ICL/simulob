...
class UserWindow: public SubWindow{
  public:
    ...
    UserWindow(std::shared_ptr<User> user, int h=0, int w=COLS-2, int y=0, int x=1);
    ~UserWindow();
...
namespace Interface{
  IntPanel setup(int status_y, int sparrow_y, int user_y, std::shared_ptr<User> user);
  void setup(IntPanel&, std::shared_ptr<OrderBook>&, std::shared_ptr<OrderSubmission>&);
  void init(IntPanel&);
