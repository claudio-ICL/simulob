...
void tController::join(){
  for (auto it = _threads.begin(); it!= _threads.end(); ++it){
    if(it->joinable()) 
      try{it->join();} catch(std::system_error& e){}
  }
}
...
BookMonitor::BookMonitor(std::shared_ptr<OrderBook> ob){
  set_book(ob);
}
...
void BookMonitor::show(){
  write("Ask best price: %8d            Ask tot volumes: %10ld\nBid best price: %8d            Bid tot volumes: %10ld",
      _book->askprice(), _book->askvolume(), _book->bidprice(), _book->bidvolume());
  while (_go){
