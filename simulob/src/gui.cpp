...
bool SubWindow::scribendum() const { return !_scribenda.empty(); }
...
UserWindow::UserWindow(std::shared_ptr<User> user, int h, int w, int y, int x)
    : UserWindow(h, w, y, x) {
  set_user(user);
}
...
void UserWindow::set_query(std::string sentence) { _query = sentence; }
void UserWindow::set_user(std::mutex *usermtx,
                          std::condition_variable *usercond) {
...
OrderCommand UserWindow::process_response() {
  ...
  try {
    ...
    switch (side) {
    case 'a':
      cmd.d = OrderDirection::ask;
      break;
    case 'A':
      cmd.d = OrderDirection::ask;
      break;
    case 'b':
      cmd.d = OrderDirection::bid;
      break;
    case 'B':
      cmd.d = OrderDirection::bid;
      break;
...
void SparrowWindow::add(const char *msg) { write(msg); }
void SparrowWindow::add(std::string s) { add(s.c_str()); }
IntPanel Interface::setup(int status_y, int sparrow_y, int user_y,
  ...
  IntPanel interface;
  interface.sparrow = std::make_shared<SparrowWindow>(user_y - sparrow_y - 2,
                                                      COLS - 2, sparrow_y, 1);
  interface.user = std::make_shared<UserWindow>(user, LINES - user_y - 1,
                                                COLS - 2, user_y, 1);
  user->set_userwindow(interface.user);
  interface.status = std::make_shared<SubWindow>(sparrow_y - status_y - 1,
                                                 COLS - 2, status_y, 1);
  return interface;
}
void Interface::setup(IntPanel &interface, std::shared_ptr<OrderBook> &ob,
...
void Interface::cdisplay(IntPanel interface) { Interface::display(interface); }
