...
MatchRes OrderBook::match_orders(std::shared_ptr<Order> &order_1,
                                 std::shared_ptr<Order> &order_2) {
  MatchRes res;
  res.transaction = false;
  if ((order_1) && (order_2)) {
    std::lock_guard<std::mutex> lock_1(*order_1->mtx());
    std::lock_guard<std::mutex> lock_2(*order_2->mtx());
    if ((order_1->status() == OrderStatus::queued) &&
        (order_2->status() == OrderStatus::queued)) {
      if ((order_1->direction() != order_2->direction()) &&
          (order_1->id() != order_2->id())) {
        std::shared_ptr<Order> &ask =
            (order_1->direction() == OrderDirection::ask) ? order_1 : order_2;
        std::shared_ptr<Order> &bid =
            (order_1->direction() == OrderDirection::bid) ? order_1 : order_2;
        ...
        if (ask->price() <= bid->price()) {
          ...
          if (ask->size() <= 0)
            ask->mark_as(OrderStatus::executed);
          if (bid->size() <= 0)
            bid->mark_as(OrderStatus::executed);
        }
      }
    }
  }
...
bool OrderBook::is_match(std::shared_ptr<Order> &order_1,
                         std::shared_ptr<Order> &order_2) {
  if ((order_1) && (order_2)) {
    std::lock_guard<std::mutex> lock_1(*order_1->mtx());
    std::lock_guard<std::mutex> lock_2(*order_2->mtx());
...
OrderQueue::OrderQueue(OrderDirection d, int opening_price, std::mutex *bookmtx,
                       std::condition_variable *bookcond)
    : _direction(d), _bestprice(BestPrice(d, opening_price)), _bookmtx(bookmtx),
      _bookcond(bookcond) {}
...
OrderQueue::OrderQueue(const OrderQueue &source)
    : std::deque<std::shared_ptr<Order>>{source}, Controller(),
      _direction(source.direction()),
      _bestprice(BestPrice(_direction, source.bestprice_value())),
      _bookmtx(source.bookmtx()), _bookcond(source.bookcond()) {}
  ...
  if (!is_empty()) {
    ...
    if (order->direction() != _direction) {
      ...
      for (auto it = this->begin();
           ...
           (_direction * order->price() <=
            _direction * (*it)->price()) //_direction is either 1 (bid queue) or
                                         //-1 (ask queue)
           ;) {
        int id = (*it)->id();
        ...
        if (mr.transaction) {
          write("Limit orders #%d(%s) and #%d(%s) match:  transaction "
                "price=%d, transaction size=%d\n",
    ...
    this->invalidate_bestprice();
    int p = (_direction == OrderDirection::ask)
                ? std::max(_bestprice.cached_price(), order->price())
                : std::min(_bestprice.cached_price(), order->price());
  ...
  while (this->go()) {
    if (!is_empty()) {
      ...
      if (alterations) {
        std::lock_guard<std::mutex> booklock(*_bookmtx);
        renew_bestprice();
        _bookcond->notify_all(); // we notify OrderBook so that it can refresh
                                 // volumes
      }
    }
  }
...
void OrderQueue::sort() {
  if (!is_empty()) {
    ...
    std::sort(this->begin(), this->end(),
              [](std::shared_ptr<Order> a, std::shared_ptr<Order> b) {
                return *a < *b;
              });
  }
}
...
OrderBook::OrderBook(int ticksize, int opening_bidprice, int opening_askprice)
  ...
  _queue = std::map<OrderDirection, OrderQueue>{
      {OrderDirection::bid,
       OrderQueue(OrderDirection::bid, opening_bidprice, &_mtx, &_cond)},
      {OrderDirection::ask,
       OrderQueue(OrderDirection::ask, opening_askprice, &_mtx, &_cond)},
  };
}
OrderBook::OrderBook(int ticksize, int opening_bidprice, int opening_askprice,
...
void OrderBook::add(std::shared_ptr<Order> &&order) {
  if (order) {
    ...
    write("Order #%d is being added on %s side; price=%d, size=%d\n",
          order->id(), order->side().c_str(), order->price(),
          order->active_size());
...
BestPrice::BestPrice(OrderDirection d, int initial_price)
    : _direction(d), _cached(initial_price) {}
BestPrice::BestPrice(const BestPrice &source)
    : _direction(source.direction()), _price(source.price()),
      _cached(source.cached_price()) {
  _order = source.order();
}
BestPrice::BestPrice(BestPrice &&source)
    : _direction(source.direction()), _price(source.price()),
      _cached(source.cached_price()) {
  _order = source.order();
}
...
int BestPrice::value() const { return (_order) ? _price : _cached; }
...
long OrderBook::askvolume() { return _queue.at(OrderDirection::ask).volume(); }
long OrderBook::bidvolume() { return _queue.at(OrderDirection::bid).volume(); }
std::random_device rd;
std::default_random_engine OrderSubmission::generator(rd());
std::discrete_distribution<int>
    OrderSubmission::distribution({0.05, 0.35, 0.40, 0.05, 0.05, 0.05, 0.05});
OrderSubmission::OrderSubmission(std::shared_ptr<OrderBook> ob,
                                 double decayrate)
    : hawkes(HawkesProcess(2)), _decayrate(decayrate) {
...
OrderCommand OrderSubmission::generate_command(Event &&event) {
  ...
  int bestprice =
      (cmd.d == OrderDirection::bid) ? _book->bidprice() : _book->askprice();
  cmd.price = std::max(0, bestprice - (cmd.d) * (_book->ticksize()) * level);
  cmd.size = std::max(0, 50 * (1 + distribution(generator)));
  // write("OrderSubmission::generate_command side=%d, level=%d, price=%d,
...
void OrderSubmission::simulate() {
  ...
  while (this->go()) {
    ...
    _book->add(
        std::make_shared<StandingOrder>(cmd.price, cmd.size, cmd.d, _subwindow),
        _decayrate);
  }
}
