...
Order::Order(int price, int size, OrderDirection direction)
    : _id(count++), _price(price), _size(size), _direction(direction),
...
void Order::mark_as(OrderStatus new_status) { _status = new_status; }
...
StandingOrder::StandingOrder(int price, int size, OrderDirection direction,
                             std::shared_ptr<SubWindow> subwindow)
    : Order{price, size, direction, subwindow} {}
...
void StandingOrder::cancel() {
  ...
  while ((_status == OrderStatus::queued) &&
  ...
  if (_status == OrderStatus::queued) {
    write("Order #%d is cancelled from %s queue; price=%d, level=%d \n", _id,
          side().c_str(), _price, 1 + distance_from_refprice());
    mark_as(OrderStatus::cancelled);
  }
}
bool StandingOrder::update() {
  if ((_status == OrderStatus::queued) && (_decayrate > 0.0)) {
    /*coefficient of exponential decay lambda is larger if the order is near the
     * best price*/
...
void StandingOrder::set_decay(BestPrice *refprice, int ticksize,
                              double decayrate) {
  _refprice = refprice;
  _ticksize = ticksize;
